...
public class VulkanDepthBuffer : VulkanDeviceDependancy, IDisposable
{
    ...
    public VulkanDepthBuffer(Vk vk, VulkanVirtualDevice device, VulkanCommandPool commandPool, uint width, uint height, SampleCountFlags sampleCount)
        : base(vk, device)
    {
        var format = device.PhysicalDevice.DepthFormat;
        this.DepthImage = new(vk, device, width, height, 1, format, ImageTiling.Optimal, ImageUsageFlags.ImageUsageDepthStencilAttachmentBit, MemoryPropertyFlags.MemoryPropertyDeviceLocalBit, sampleCount);
