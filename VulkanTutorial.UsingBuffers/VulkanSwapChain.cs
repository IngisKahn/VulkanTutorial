...
public class VulkanSwapChain : IDisposable
{
    ...
    public VulkanSwapChain(Vk vk, IWindow window, VulkanInstance instance, VulkanPhysicalDevice physicalDevice,
        VulkanVirtualDevice device, in SurfaceKHR surface)
    {
        this.device = device;
        var swapChainSupport = physicalDevice.QuerySwapChainSupport();

        var surfaceFormat =
            swapChainSupport.Formats.FirstOrDefault(f => f.Format == Format.B8G8R8A8Unorm, swapChainSupport.Formats[0]);
        ...
        this.GraphicsPipeline = new(vk, device, this);
